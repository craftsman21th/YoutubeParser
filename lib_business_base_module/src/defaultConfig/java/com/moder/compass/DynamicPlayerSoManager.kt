...
private const val remoteLibDownloadFile = "remotePlayerLibTemp.zip"
private const val REMOTE_LIB_SAVE_FILE = "remotelib_player_315"

private const val STATUS_LOADING = "1"
private const val STATUS_LOADED = "0"

private const val CACHE_SO_FILE = "dynamicsofile_player"
private const val KEY_SO_PLAYER_FILE_LENGTH = "key_dynamic_so_remote_lib_player_length"
...
class DynamicPlayerSoManager private constructor(
            ...
            override fun onSuccess(file: File, fileMd5: String) {
                ...
                if (fileMd5 == MD5_REMOTE_LIB){
                    ...
                    calculateMD5(File(upZipDestFile, REMOTE_LIB_SAVE_FILE)).let {
                        GlobalConfig.getInstance().putString(KEY_SO_PLAYER_FILE_LENGTH, it)
                    }
                    setLoadingState(false)
                    notifyLoadSuccess()
                }else{
    ...
    private fun initDynamicSOFiles() {
        soInstalled = DynamicSoLauncher.initDynamicSoConfig(
            context = context,
            soPath = File(upZipDestFile, REMOTE_LIB_SAVE_FILE).absolutePath
        )
    }
    ...
    fun tryLoad(pageTag : String = "",listener: DynamicSoLoadListener? = null) {
        ...
        if (!loading || needReDownload) {
            val md5 = GlobalConfig.getInstance().getString(KEY_SO_PLAYER_FILE_LENGTH)
    ...
    fun hasLoaded(): Boolean {
        return soInstalled && checkRemoteFile(GlobalConfig.getInstance().getString(KEY_SO_PLAYER_FILE_LENGTH))
    }
    ...
    companion object {
        private const val TAG = "dynamic_so_player"
        ...
        private val URL_REMOTE_LIB =
            "${HostURLManager.getDefaultRemoteResourcePrefix()}/moder/cdn_dynamic_resource/remotelib_player_315.zip"
        private const val MD5_REMOTE_LIB = "06fdb79f403ff66e3bd1a2ede45b4f90"
