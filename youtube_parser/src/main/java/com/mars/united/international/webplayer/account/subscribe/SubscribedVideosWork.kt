...
import com.mars.united.international.webplayer.parser.work.callback.SubscribedVideosWorkCallback
 ...
 * @Desc 已关注的人发布的videos列表 任务
 */
class SubscribedVideosWork(
    ...
    private data class CallbackWrapper(
        val page: Int,
        val pageCount: Int,
        val callback: SubscribedVideosWorkCallback
    )
    ...
    override suspend fun onWorkInit(): Boolean = withContext(Dispatchers.IO) {
        val ytInitialData = commonWork.getYtInitialData("https://m.youtube.com/feed/subscriptions")
        // 解析 请求地址
        if (apiUrl.isBlank()) {
            apiUrl = ytInitialData["contents"]["singleColumnBrowseResultsRenderer"]["tabs"]
                .get(INT_0)["tabRenderer"]["endpoint"]["commandMetadata"]["webCommandMetadata"]["apiUrl"]
                .asString ?: ""
        }
        // 解析 请求令牌
        if (continuation.isBlank()) {
            continuation = ytInitialData["contents"]["singleColumnBrowseResultsRenderer"]["tabs"]
                .get(INT_0)["tabRenderer"]["content"]["sectionListRenderer"]["continuations"]
    ...
    private fun JsonCaller?.extractContinuationToken(): String {
        return this["continuationContents"]["sectionListContinuation"]["continuations"]
            .map { this["nextContinuationData"] }[INT_0]["continuation"]
            .asString ?: ""
    }
    ...
    private fun JsonCaller?.extractResult(): List<JsonCaller> {
        ...
        (if (isFirstRequest) {
            this["continuationContents"]["sectionListContinuation"]["contents"].map {
                this["itemSectionRenderer"]["contents"][INT_0]["compactVideoRenderer"]
            }
        } else {
     ...
     * >
     */
    fun fetch(page: Int, pageCount: Int = DEFAULT_PAGE_COUNT, callback: SubscribedVideosWorkCallback) {
