...
import com.mars.united.international.webplayer.parser.work.callback.RecommendWorkCallback
...
class RecommendWork(
    ...
    private data class CallbackWrapper(
        val page: Int,
        val pageCount: Int,
        val callback: RecommendWorkCallback
    )
    ...
    override suspend fun getUrl(): String {
        return "https://www.youtube.com/youtubei/v1/next?key=${CommonRepo.get().apiKey}"
    }
    ...
    override suspend fun getRequestBody(): JsonCaller {
        ...
        params["context"] = JsonCaller.createObject().apply {
            this["client"] = JsonCaller.createObject().apply {
                this["gl"] = commonRepo.ytConfig["GL"] ?: "US"
                this["hl"] = commonRepo.ytConfig["HL"].asString ?: ""
                this["clientName"] = commonRepo.ytConfig["INNERTUBE_CLIENT_NAME"].asString ?: "MWEB"
                this["clientVersion"] = commonRepo.ytConfig["INNERTUBE_CLIENT_VERSION"].asString ?: "2.20230616.01.00"
            }
        }
        // 区分首次搜索和加载更多，构造不同参数
        if (isFirstRequest) {
            params["videoId"] = videoId
            params["racyCheckOk"] = false
        } else {
    ...
    private fun JsonCaller?.extractContinuationToken(isFirstRequest: Boolean): String {
        return if (isFirstRequest) {
            this["contents"]["singleColumnWatchNextResults"]["results"]["results"]["contents"]
                .filter {
                    this["itemSectionRenderer"]["sectionIdentifier"].asString == "related-items"
            ...
            this["onResponseReceivedEndpoints"].filter {
                has("appendContinuationItemsAction")
            }[0]["appendContinuationItemsAction"]["continuationItems"]
                .filter {
                    this["continuationItemRenderer"]["continuationEndpoint"] != null
                }[0]["continuationItemRenderer"]["continuationEndpoint"]["continuationCommand"]["token"]
                .asString ?: ""
        }
    }
    ...
    override fun onCallback(errCode: Int?) {
        Log.e(TAG, "recommend result - continuationToken:${lastContinuationToken}")
        if (errCode != null) {
            // 请求失败回调
            Log.e(TAG, "recommend result Error - errCode: ${errCode}")
        } else {
            Log.e(
                TAG, "recommend result Success - " +
    ...
    private fun JsonCaller?.extractResult(isFirstRequest: Boolean): List<JsonCaller> {
        ...
        (if (isFirstRequest) {
            this["contents"]["singleColumnWatchNextResults"]["results"]["results"]["contents"].filter {
                this["itemSectionRenderer"]["sectionIdentifier"].asString == "related-items"
            }[0]["itemSectionRenderer"]["contents"]
                .map {
                    this["compactAutoplayRenderer"]["contents"]["videoWithContextRenderer"]
                        ?: this["videoWithContextRenderer"]
                }
        } else {
            this["onResponseReceivedEndpoints"][0]["appendContinuationItemsAction"]["continuationItems"]
                .map {
                    this["videoWithContextRenderer"]
                }
        }).forEach {
     ...
     * >
     */
    fun fetch(page: Int, pageCount: Int = DEFAULT_PAGE_COUNT, callback: RecommendWorkCallback) {
